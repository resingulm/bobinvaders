...
class Player {
    ...
    shoot() {
        if (this.canShoot && keys[' ']) {
            playerBullets.push(new Bullet(this.x + this.width / 2 - BULLET_WIDTH / 2, this.y, -BULLET_SPEED, 'player'));
            sounds.playerShoot();
            this.canShoot = false;
        }
...
class Alien {
    ...
    shoot() {
        if (this.canShoot() && Math.random() < ALIEN_SHOOT_CHANCE) {
            alienBullets.push(new Bullet(this.x + this.width / 2 - BULLET_WIDTH / 2, this.y + this.height, BULLET_SPEED, 'alien'));
            sounds.alienShoot();
        }
    }
}
